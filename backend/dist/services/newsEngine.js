...
exports.triggerChaosEvent = exports.triggerManualNews = exports.triggerAINews = exports.activeNewsModifiers = exports.NEWS_TEMPLATES = void 0;
const db_js_1 = require("../config/db.js");
const index_js_1 = require("../schema/index.js");
const drizzle_orm_1 = require("drizzle-orm");
const socketHandler_js_1 = require("../sockets/socketHandler.js");
exports.NEWS_TEMPLATES = [
    // Technology News
    {
...
exports.activeNewsModifiers = new Map();
const triggerAINews = async (categoryFilter, sectorFilter) => {
    let filtered = exports.NEWS_TEMPLATES;
    ...
    if (filtered.length === 0) {
        filtered = exports.NEWS_TEMPLATES;
    }
    ...
    await db_js_1.db.insert(index_js_1.newsEvents).values(newEventData);
    ...
    exports.activeNewsModifiers.set(targetKey, {
        impact: template.expectedImpact,
        expiresAt: now + 90000 // 90 seconds
    });
    // Fetch created row
    const createdList = await db_js_1.db.select().from(index_js_1.newsEvents).where((0, drizzle_orm_1.eq)(index_js_1.newsEvents.id, id));
    const created = createdList[0];
    (0, socketHandler_js_1.broadcastNewsEvent)(created);
    console.log(`📰 [NEWS RELEASED]: ${created.headline} (Impact: ${created.expectedImpact}%)`);
    return created;
};
exports.triggerAINews = triggerAINews;
const triggerManualNews = async (data) => {
    const now = Date.now();
    ...
    if (data.affectedCompanyId) {
        exports.activeNewsModifiers.set(`COMP_${data.affectedCompanyId}`, { impact: data.expectedImpact, expiresAt: now + 90000 });
    }
    else if (data.affectedSector) {
        exports.activeNewsModifiers.set(`SECTOR_${data.affectedSector}`, { impact: data.expectedImpact, expiresAt: now + 90000 });
    }
    else {
        exports.activeNewsModifiers.set('GLOBAL', { impact: data.expectedImpact, expiresAt: now + 90000 });
    }
    const createdList = await db_js_1.db.select().from(index_js_1.newsEvents).where((0, drizzle_orm_1.eq)(index_js_1.newsEvents.id, id));
    const created = createdList[0];
    (0, socketHandler_js_1.broadcastNewsEvent)(created);
...
exports.triggerManualNews = triggerManualNews;
const triggerChaosEvent = async (type) => {
    ...
    if (type === 'CRASH') {
        ...
        for (const c of allCompanies) {
            ...
            await db_js_1.db.update(index_js_1.companies).set({ previousPrice: c.currentPrice, currentPrice: newPrice }).where((0, drizzle_orm_1.eq)(index_js_1.companies.id, c.id));
        }
    }
    else if (type === 'BOOM') {
        ...
        await db_js_1.db.update(index_js_1.companies).set({ previousPrice: randomComp.currentPrice, currentPrice: newPrice }).where((0, drizzle_orm_1.eq)(index_js_1.companies.id, randomComp.id));
    }
    ...
    const id = `chaos-${now}`;
    await db_js_1.db.insert(index_js_1.newsEvents).values({
        id,
    ...
    const updatedCompanies = await db_js_1.db.select().from(index_js_1.companies);
    (0, socketHandler_js_1.broadcastMarketUpdate)(updatedCompanies);
    (0, socketHandler_js_1.broadcastChaosEvent)({ title, message, type });
    console.log(`💥 [CHAOS EVENT FIRED]: ${title}`);
};
exports.triggerChaosEvent = triggerChaosEvent;
